...
function useReveal(rootMargin) {
  ...
  React.useEffect(() => {
    ...
    const obs = new IntersectionObserver(([e]) => {
      ...
      obs.disconnect();
    }, { threshold: 0.12, rootMargin: rootMargin || '0px 0px -140px 0px' });
    obs.observe(el);
...
function useStagger(rootMargin) {
  const ref = React.useRef(null);
  React.useEffect(() => {
    const el = ref.current;
    if (!el) return;
  ...
  return (
    <section style={{ background: '#010101', position: 'relative', overflow: 'hidden', color: 'white', '--ink': '#ffffff', '--ink-700': 'rgba(255,255,255,.72)', '--ink-500': 'rgba(255,255,255,.40)' }}>
      <div className="container" style={{ position: 'relative', zIndex: 2 }}>
        ...
        <div ref={introRef} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center', marginBottom: 72 }}>
          <div>
...
function HowToWin({ onStepClick }) {
  ...
  return (
    <section style={{ padding: '100px 0 96px', background: '#0a0a0a' }}>
      <div className="container">
        <div ref={headerRef} className="center" style={{ marginBottom: 56 }}>
          <div className="eyebrow" style={{ marginBottom: 14 }}>How to Win</div>
          <h2 className="h-display" style={{ maxWidth: 960, margin: '0 auto 20px', textWrap: 'balance', color: '#f5f1eb', fontSize: 'clamp(44px, 6.5vw, 88px)' }}>
            Three steps. Up to <span style={{ color: 'var(--coral)' }}>135</span> points.
          </h2>
          <p className="lede" style={{ color: 'rgba(255,255,255,.45)' }}>
            Only Step 1 is required. The more you stack, the better your odds.
          </p>
        </div>

        <div ref={cardsRef} style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, alignItems: 'stretch', gridAutoRows: '1fr' }}>
          {steps.map((s) => {
  ...
  return (
    <section id="faq" className="section-dark" style={{ padding: '100px 0', position: 'relative', overflow: 'hidden' }}>
      ...
      <div className="container" style={{ maxWidth: 820, position: 'relative', zIndex: 2 }}>
        <div ref={faqHeaderRef} className="center" style={{ marginBottom: 48 }}>
