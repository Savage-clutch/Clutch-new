...
function EnterSection({ formRef }) {
  ...
  return (
    <section ref={formRef} style={{ background: 'var(--ink-150)', position: 'relative', overflow: 'hidden', padding: '100px 0' }}>
      <div className="container" style={{ position: 'relative', zIndex: 2, maxWidth: 1000 }}>
        <div className="center" style={{ marginBottom: 48 }}>
          <h2 className="h-display" style={{ maxWidth: 880, margin: '0 auto 18px', textWrap: 'balance' }}>
            Get maximum points by completing all three steps.
          </h2>
          <p className="lede">
            Start with Step 1 to officially enter. Then complete Steps 2 and 3 for the best shot at driving home in the 2024 BMW X5.
          </p>
            ...
            <div style={{ fontSize: 13, color: 'var(--ink-700)', fontWeight: 600 }}>
              {step1Done ? (
                ...
                <>Points available: <strong style={{ color: 'var(--coral)', fontWeight: 900, fontSize: 16 }}>135</strong>
                  <span style={{ color: 'var(--ink-500)', fontWeight: 500 }}> — start with Step 1</span></>
              )}
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          >
            {!submitted ? (
              <>
                <div style={{ display: 'inline-block', padding: '6px 14px', border: '1.5px solid var(--coral)', borderRadius: 999, color: 'var(--coral)', fontSize: 11, fontWeight: 800, letterSpacing: '.14em', textTransform: 'uppercase', marginBottom: 18 }}>
                  Required
                </div>
                <div style={{ fontSize: 15, color: 'var(--ink-700)', lineHeight: 1.55, marginBottom: 24 }}>
                  <strong style={{ color: 'var(--ink)' }}>This is your official entry into the giveaway.</strong> Fill in your details below to be eligible to win.
                </div>
                ...
                <form onSubmit={submit}>
                  ...
                  <div style={{ marginBottom: 8 }}>
                    <Field label="Social handle (Instagram or TikTok)" value={handle} setValue={setHandle} placeholder="@yourhandle" />
                  </div>
                  <div style={{ fontSize: 12, color: 'var(--ink-700)', marginBottom: 20 }}>
                    Use the same handle you'll post from in Step 3 — that's how bonus points are tracked.
                  </div>
          ...
          >
            <div style={{ fontSize: 15, color: 'var(--ink-700)', lineHeight: 1.6, marginBottom: 22 }}>
              Get a <strong style={{ color: 'var(--ink)' }}>free, no-obligation instant cash offer</strong> for any vehicle you own. It takes about <strong style={{ color: 'var(--ink)' }}>1 minute</strong> to get an instant cash offer — earn <strong style={{ color: 'var(--ink)' }}>100 points for every vehicle you own</strong>. Submit as many vehicles as you like.¹
            </div>
            ...
            <div style={{ display: 'inline-flex', background: 'var(--ink-150)', borderRadius: 999, padding: 4, marginBottom: 14, fontSize: 12, fontWeight: 700 }}>
              <button onClick={() => setVinMode(false)} style={{
                padding: '6px 14px', borderRadius: 999, border: 'none', cursor: 'pointer',
                background: !vinMode ? 'white' : 'transparent',
                color: !vinMode ? 'var(--ink)' : 'var(--ink-500)',
                boxShadow: !vinMode ? '0 2px 8px -2px rgba(0,0,0,.1)' : 'none',
                fontFamily: 'inherit', letterSpacing: '.02em',
                transition: 'background .2s, color .2s',
              }}>Licence plate</button>
              <button onClick={() => setVinMode(true)} style={{
                padding: '6px 14px', borderRadius: 999, border: 'none', cursor: 'pointer',
                background: vinMode ? 'white' : 'transparent',
                color: vinMode ? 'var(--ink)' : 'var(--ink-500)',
                boxShadow: vinMode ? '0 2px 8px -2px rgba(0,0,0,.1)' : 'none',
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 12 }}>
              <Field
                label={vinMode ? 'VIN' : 'Licence plate'}
                value={plate}
                setValue={setPlate}
                placeholder={vinMode ? 'e.g. 1HGCM82633A123456' : 'e.g. ABCD 123'}
              />
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
              <button className="btn btn-primary" onClick={startOffer} disabled={!step1Done || !plate.trim() || !postal.trim()}
                style={{ fontSize: 17, padding: '16px 30px', opacity: (!step1Done || !plate.trim() || !postal.trim()) ? 0.5 : 1 }}>
                Start offer
              </button>
              {step2Visited && (
                <div style={{ fontSize: 12, color: 'var(--ink-700)', fontWeight: 600, fontStyle: 'italic' }}>
                  Opened in a new tab · Finish the offer there, then come back for Step 3.
                </div>
              )}
            </div>
          </StepCard>
          ...
          >
            <div style={{ fontSize: 15, color: 'var(--ink-700)', lineHeight: 1.6, marginBottom: 22 }}>
              Tap any of the 3 Clutch ads below. Leave a comment with a quirk, something interesting or funny about your car. Earn <strong style={{ color: 'var(--ink)' }}>10 points per ad</strong> — up to <strong style={{ color: 'var(--ink)' }}>30 points total</strong>. Post from the same handle you used in Step 1.
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 16 }}>
              {[0, 1, 2].map(i => (
                <AdCard key={i} idx={i} visited={visitedAds[i]} disabled={!step1Done} onClick={() => step1Done && visitAd(i)} />
              ))}
            </div>
...
function StepCard({ step, duration, title, pts, ptsLabel, required, complete, highlighted, dimmed, nextUp, icon, iconRotate = 0, children }) {
  ...
  return (
    <div id={`step-${step}`} className="card step-card" style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 24, marginBottom: 18, position: 'relative', zIndex: 2 }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: 'var(--ink-700)', fontWeight: 600, marginBottom: 14 }}>
  ...
  return (
    <section style={{ padding: '100px 0' }}>
      <div className="container-narrow center">
        <div className="eyebrow" style={{ marginBottom: 14 }}>Shop Clutch Certified</div>
...
function TermsAndConditions() {
  const sections = [
    ...
    },
    {
      ...
      body: (
        <>
          <p>One (1) grand prize: a Clutch Certified [2024 BMW X1] (or a vehicle of equivalent or greater approximate retail value as confirmed by Clutch prior to launch). Approximate retail value: $50,000 CAD.</p>
  ...
  return (
    <section id="terms" style={{ padding: '64px 0 80px', background: 'var(--ink-150)', borderTop: '1px solid rgba(0,0,0,.06)' }}>
      <div className="container" style={{ maxWidth: 880 }}>
        ...
        <div style={{
          fontSize: 12, lineHeight: 1.7, color: 'var(--ink-700)',
        }}>
          {sections.map((s, i) => (
