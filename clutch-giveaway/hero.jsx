...
function Hero({ deadlineIso, onCtaClick }) {
  ...
  useEffect(() => {
    ...
    const blit = (img, frame) => {
      // Measure target fresh at the exact moment interpolation begins — layout is stable by now
      if (frame === Math.floor(TOTAL * 0.4)) {
        targetRect = inPageCanvas.getBoundingClientRect();
      }

      const ctx = introCanvas.getContext('2d');
      const cw = introCanvas.width, ch = introCanvas.height;
      const iw = img.naturalWidth, ih = img.naturalHeight;
      const containScale = Math.min(cw / iw, ch / ih);
      const fullW = iw * containScale, fullH = ih * containScale;
      const fullX = (cw - fullW) / 2, fullY = (ch - fullH) / 2;

      let x = fullX, y = fullY, w = fullW, h = fullH;

      // Last 40%: interpolate toward in-page canvas — cubic ease-in-out so the shrink starts
      // slowly, accelerates through the middle, and lands gently rather than lurching.
      const SCALE_START = Math.floor(TOTAL * 0.4);
      if (frame >= SCALE_START && targetRect) {
        const t = (frame - SCALE_START) / (TOTAL - SCALE_START);
        const ease = t < 0.5
          ? 4 * t * t * t
      ...
      lastDrawRectRef.current = { x, y, w, h };
    };
  ...
  // When settling starts: stamp last frame at same screen position as overlay, crossfade out
  useEffect(() => {
    if (phase !== 'settling') return;
    const overlay = overlayRef.current;
    const canvas = canvasRef.current;
    const img = lastFrameRef.current;
    const lastDraw = lastDrawRectRef.current;
    if (!overlay || !canvas || !img || !lastDraw) return;
    ...
    // Transition is already set by React when phase='settling'; just trigger the opacity drop
  ...
  return (
    <>
      {/* ── FULL-SCREEN INTRO OVERLAY — always mounted, hidden via opacity once done ── */}
      <div ref={overlayRef} style={{
        position: 'fixed', inset: 0, zIndex: 9999, background: '#010101', overflow: 'hidden',
        opacity: phase === 'done' ? 0 : 1,
        pointerEvents: phase === 'done' ? 'none' : 'auto',
        transition: phase === 'settling' ? 'opacity 0.4s ease' : 'none',
      }}>
        <canvas ref={introCanvasRef} style={{ width: '100%', height: '100%', display: 'block' }} />
      </div>

    <section className="section-dark" style={{ padding: '140px 0 0', position: 'relative', background: '#010101', overflow: 'hidden', minHeight: '100vh' }}>
      <div className="container" style={{ position: 'relative', zIndex: 2 }}>

        {/* Always rendered so layout is stable — animation classes always applied, play-state controls when they fire */}
        <div style={{ pointerEvents: phase === 'done' ? 'auto' : 'none' }}>
          {/* Title */}
          <h1 className="h-display center hero-animate hero-title" style={{ maxWidth: 1000, margin: '0 auto 28px', textWrap: 'balance', animationPlayState: phase === 'done' ? 'running' : 'paused' }}>
            Win a <span style={{ whiteSpace: 'nowrap' }}>Clutch Certified</span><br />2025 Audi Q7.
          </h1>

          {/* Countdown */}
          <div className="center hero-animate hero-clock" style={{ marginBottom: 28, animationPlayState: phase === 'done' ? 'running' : 'paused' }}>
          ...
          <p className="lede center hero-animate hero-lede" style={{ margin: '0 auto 28px', animationPlayState: phase === 'done' ? 'running' : 'paused' }}>
            Enter for a chance to win a free 2025 Audi Q7 and experience Clutch Certified first hand.
          </p>

          {/* CTA */}
          <div className="center hero-animate hero-cta" style={{ marginBottom: 60, animationPlayState: phase === 'done' ? 'running' : 'paused' }}>
